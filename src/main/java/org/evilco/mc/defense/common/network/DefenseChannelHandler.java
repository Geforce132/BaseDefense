...
public class DefenseChannelHandler extends MessageToMessageCodec<FMLProxyPacket, AbstractDefensePacket> {
	...
	protected void decode (ChannelHandlerContext ctx, FMLProxyPacket msg, List<Object> out) throws Exception {
		...
		if (packet instanceof DefenseStationRegisterUserPacket) {
			...
			// verify owner
			if (!defenseStationTileEntity.isWhitelisted (player)) {
				// forcefully close screen
				player.closeScreen ();
			...
			if (entity == null) {
				// notify client
				DefenseModification.getProxy ().getChannels ().get (Side.SERVER).attr (FMLOutboundHandler.FML_MESSAGETARGET).set (FMLOutboundHandler.OutboundTarget.PLAYER);
				DefenseModification.getProxy ().getChannels ().get (Side.SERVER).attr (FMLOutboundHandler.FML_MESSAGETARGETARGS).set (player);
				DefenseModification.getProxy ().getChannels ().get (Side.SERVER).writeAndFlush (new DefenseStationRegisterUserErrorPacket ());
			...
			// add user
			if (!userPacket.isBlacklist ())
				defenseStationTileEntity.addWhitelist (entity.getPersistentID (), entity.getDisplayName ());
			else
				defenseStationTileEntity.addBlacklist (entity.getPersistentID (), entity.getDisplayName ());

			// send success packet
			DefenseModification.getProxy ().getChannels ().get (Side.SERVER).attr (FMLOutboundHandler.FML_MESSAGETARGET).set (FMLOutboundHandler.OutboundTarget.PLAYER);
			DefenseModification.getProxy ().getChannels ().get (Side.SERVER).attr (FMLOutboundHandler.FML_MESSAGETARGETARGS).set (player);
			DefenseModification.getProxy ().getChannels ().get (Side.SERVER).writeAndFlush (new DefenseStationRegisterUserSuccessPacket ());
			...
			if (unregisterPacket.getUserID ().equals (player.getPersistentID ())) return;

			// add user
			if (!unregisterPacket.isBlacklist ())
				defenseStationTileEntity.removeWhitelist (unregisterPacket.getUserID ());
			else
				defenseStationTileEntity.removeBlacklist (unregisterPacket.getUserID ());
