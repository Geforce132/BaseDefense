 ...
 */
package org.evilco.mc.defense.client.gui.generic;
...
public class DefenseStationGui extends GuiScreen implements ReturnAwareGuiTextField.ReturnListener {
	...
	/**
	 * Indicates the list being edited right now.
	 */
	...
	protected void mouseClicked (int par1, int par2, int par3) {
		...
		int offsetY = 0;
		Iterator<Map.Entry<UUID, String>> iterator = (this.mode ? this.tileEntity.getBlacklist () : this.tileEntity.getWhitelist ()).entrySet ().iterator ();
		...
		while (iterator.hasNext ()) {
			...
			if (par2 >= correctY && par2 <= (correctY + 8)) {
				...
				DefenseModification.getProxy ().getChannels ().get (Side.CLIENT).attr (FMLOutboundHandler.FML_MESSAGETARGET).set (FMLOutboundHandler.OutboundTarget.TOSERVER);
				DefenseModification.getProxy ().getChannels ().get (Side.CLIENT).writeOutbound (new DefenseStationUnregisterPacket (element.getKey (), this.mode, this.tileEntity.xCoord, this.tileEntity.yCoord, this.tileEntity.zCoord));
				break;
			}
	...
	public void onReturn () {
		...
		// send packet to server
		DefenseModification.getProxy ().getChannels ().get (Side.CLIENT).attr (FMLOutboundHandler.FML_MESSAGETARGET).set (FMLOutboundHandler.OutboundTarget.TOSERVER);
		DefenseModification.getProxy ().getChannels ().get (Side.CLIENT).writeOutbound (new DefenseStationRegisterUserPacket (text, this.mode, this.tileEntity.xCoord, this.tileEntity.yCoord, this.tileEntity.zCoord));
	}
